...
const NavLink = ({ to, icon: Icon, label, badge, onClose }) => {
  const { pathname } = useLocation();
  const active = pathname === to;
  return (
    <Link to={to} onClick={onClose} className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${active ? "bg-amber-50 text-amber-700 font-bold" : "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900"}`}>
  ...
  return (
    <div className="w-64 h-screen overflow-y-auto bg-white border-r border-zinc-200 flex flex-col">
      ...
      <div className="px-5 pb-3">
        <span className="inline-block text-[10px] font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md">Artisan Studio</span>
      </div>

      <nav className="flex-1 px-3 py-4 space-y-5">
        <div className="space-y-1">
            <NavLink to="/dashboard" icon={Home} label="Dashboard" onClose={onClose} />
          </div>

        <div>
          <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest px-3 mb-2">Quick Actions</p>
          <div className="space-y-1">
            <NavLink to="/add-product" icon={Plus} label="Add Product" onClose={onClose} />
            <NavLink to="/add-heritage" icon={Zap} label="Add Heritage" onClose={onClose} />
            <NavLink to="/add-remedy" icon={Leaf} label="Add Remedy" onClose={onClose} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            <NavLink to="/heritage" icon={Compass} label="Heritage" onClose={onClose} />
            <NavLink to="/products" icon={ShoppingBag} label="Marketplace" onClose={onClose} />
            <NavLink to="/remedies" icon={Leaf} label="Remedies" onClose={onClose} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            <NavLink to="/creator-studio" icon={Palette} label="Creator Studio" onClose={onClose} />
            <NavLink to="/showcase" icon={Sparkles} label="My Showcase" onClose={onClose} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            <NavLink to="/order-command" icon={Package} label="Order Command" badge={pendingOrders} onClose={onClose} />
            <NavLink to="/growth-insights" icon={BarChart3} label="Growth Insights" onClose={onClose} />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            <NavLink to="/customer-connect" icon={MessageCircle} label="Customer Connect" onClose={onClose} />
            <NavLink to="/smart-suggestions" icon={Brain} label="Smart Suggestions" onClose={onClose} />
          </div>
        </div>
      </nav>

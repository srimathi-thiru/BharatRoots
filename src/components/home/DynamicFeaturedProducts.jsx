...
const DynamicFeaturedProducts = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const cached = sessionStorage.getItem("bharatroots_featuredProducts");
      if (cached) {
      ...
      // 2. Fetch from database if cache doesn't exist or just to hydrate
      ...
      setProducts(data);
      sessionStorage.setItem("bharatroots_featuredProducts", JSON.stringify(data));
      setLoading(false);
    } catch (error) {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {loading && products.length === 0 ? (
            ...
            Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="h-[450px] rounded-[2rem] bg-slate-200 animate-pulse w-full"></div>
            ))
          ) : (
